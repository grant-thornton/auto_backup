...
try:
    ...
    raise ImportError('This module needs pysftp to automaticly write backups '
                      'to the FTP through SFTP. Please install pysftp on your'
                      ' system. (sudo pip install pysftp)')
...
class db_backup(models.Model):
    ...
    folder = fields.Char(
        ...
        help='Absolute path for storing the backups', required='True',
        default='/odoo/backups')
    backup_type = fields.Selection(
        [('zip', 'Zip'), ('dump', 'Dump')], 'Backup Type', required=True,
        default='zip')
    autoremove = fields.Boolean(
        'Auto. Remove Backups',
        help='If you check this option you can choose to automaticly remove '
             'the backup after xx days')
    days_to_keep = fields.Integer(
        'Remove after x days',
        help="Choose after how many days the backup should be deleted. For "
             "example:\nIf you fill in 5 the backups will be removed after 5 "
             "days.", required=True)

    # Columns for external server (SFTP)
    sftp_write = fields.Boolean(
        'Write to external server with sftp',
        help="If you check this option you can specify the details needed to "
        "write to a remote server with SFTP.")
    ...
    sftp_password = fields.Char(
        ...
        'made with. This is the password from the user on the external server')
    days_to_keep_sftp = fields.Integer(
        'Remove SFTP after x days',
        help='Choose after how many days the backup should be deleted from '
    ...
    _constraints = [(_check_db_exist,
                    _('Error ! No such database exists!'), [])]
    ...
    def test_sftp_connection(self, context=None):
        ...
        for rec in self:
            ...
            try:
                ...
                messageContent = "Everything seems properly set up for "
                "FTP back-ups!"
            except Exception, e:
        ...
        for rec in conf_ids:
            ...
            if rec.name in db_list:
                try:
                    ...
                    raise Warning(_('Error creating directory'))
                ...
                _logger.debug("database %s doesn't exist on "
                              "http://%s:%s" % (
                ...
                try:
                    ...
                    try:
                        ...
                        for dirElement in pathToWriteTo.split('/'):
                            ...
                            try:
                                ...
                                _logger.info('(Part of the) path didn\'t '
                            ...
                            if os.path.isfile(fullpath):
                                if not srv.exists(f):
                                    _logger.info(
                                        ...
                                        'FTP Server ------ '
                                        'Copying file' % fullpath)
                                    srv.put(fullpath)
                                    _logger.info(
                                        'Copying File % s------ '
                                        'success' % fullpath)
                                else:
                                    _logger.debug('File %s already exists on '
                    ...
                    for file in srv.listdir(pathToWriteTo):
                        if rec.name in file:
                            ...
                            # Get the timestamp from the file on the
                            # external server
                            ...
                            if delta.days >= rec.days_to_keep_sftp:
                                ...
                                if srv.isfile(fullpath) and\
                                   (".dump" in file or '.zip' in file):
                                    _logger.info("Delete too old file from "
                                                 "SFTP servers: " + file)
                                    srv.unlink(file)
                    # Close the SFTP session.
                    srv.close()
                except Exception, e:
                    _logger.debug('Exception! We couldn\'t back up to the '
                                  'FTP server..')
                    # At this point the SFTP backup failed. We will
                    # now check if the user wants
            ...
            if rec.autoremove:
                ...
                for f in os.listdir(dir):
                    ...
                    # (Makes it possible to save different databases in
                    # the same folder)
